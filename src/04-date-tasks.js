...
function parseDataFromRfc2822(value) {
  const parseData = Date.parse(value);
  if (!Number.isNaN(parseData)) {
    return new Date(parseData);
  }
  return null;
}
...
function timeSpanToString(startDate, endDate) {
  ...
  const formattedHourse = String(hours).padStart(2, '0');
  const formattedMinutes = String(minutes).padStart(2, '0');
  const formattedSeconds = String(second).padStart(2, '0');
  const formattedMilliseconds = String(milliseconds).padStart(3, '0');

  return `${formattedHourse}:${formattedMinutes}:${formattedSeconds}.${formattedMilliseconds}`;
}
